...
public class VoucherDAO {
    ...
    public boolean useVoucherForOrder(int voucherId, int orderId, long discountAmount) {
    Connection conn = null;
    try {
        conn = new DBContext().getConnection();
        conn.setAutoCommit(false);
        ...
        // 2. Thêm record vào VoucherUsage
        String usageSql = "INSERT INTO VoucherUsage (VoucherId, OrderId, DiscountAmount, UsedAt) VALUES (?, ?, ?, GETDATE())";
        PreparedStatement usageStmt = conn.prepareStatement(usageSql);
        usageStmt.setInt(1, voucherId);
        usageStmt.setInt(2, orderId);
        usageStmt.setLong(3, discountAmount);
        ...
        // 3. Cập nhật UsedQuantity
        String updateSql = "UPDATE Voucher SET UsedQuantity = UsedQuantity + 1 WHERE Id = ?";
        ...
        conn.commit();
        System.out.println("✅ Voucher usage recorded successfully: " + voucherId);
        return true;
